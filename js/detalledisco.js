...
fetch(url)
    ...
    .then(function(data){
        console.log(data);
        let discoDatos = `<article class= "detalle-disco"> <a class="nombre-disco" href="./detalledisco.html?id=${data.id}">${data.title}</a> <img src="${data.cover}" alt="${data.title}"> <article class="bloque-disco-datos"> <a href="./detallesartista.html?id=${data.artist.id}">${data.artist.name}
        </a> <a href="./detail-genres.html?id=${data.genres.data.id}>${data.genres.data.name}</a><p> Fecha de publicacion del disco: ${data.release_date}</p></article>`
